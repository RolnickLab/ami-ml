...
def save_track(image_dir, data_images, data_annot, idx, model, img_resize, device):
    ...
    for i in range(len(image2_annot)):
        for j in range(len(image1_annot)):
            ...
            tracking_cost    = TrackingCost(img1_moth, img2_moth,\
                                            image1_annot[j], image2_annot[i], \
                                            model, img_diagonal, weights, \
                                            img_resize, device)
...
def tracking(args):
	...
	COST_THR     = 10000      # cost thresholding for removing false tracks: no rejection right now
	...
	model         = nn.Sequential(*list(model.children())[:-1]) # only getting the last feature layer
