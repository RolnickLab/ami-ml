...
def category_map(args):
    ...
    species_list = list(set(data["gbif_species"]))
...
def taxon_hierarchy(args):
    ...
    for indx in data.index:
        if (
            data["gbif_species"][indx] not in taxon_hierarchy.keys()
            and data["gbif_species"][indx] != "NotAvail"
        ):
            taxon_hierarchy[data["gbif_species"][indx]] = [
                data["genus"][indx],
                data["family"][indx],
            ]
