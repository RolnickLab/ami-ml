...
def create_dataset_split(args):
    ...
    for _, row in species_list.iterrows():
        ...
        if os.path.isdir(os.path.join(root_dir, family, genus, species)):
            image_data = glob.glob(
                os.path.join(root_dir, family, genus, species, "*.jpg")
            )
    ...
    train_data.to_csv(
        os.path.join(write_dir, args.filename + "_train-split.csv"), index=False
    )
    val_data.to_csv(
        os.path.join(write_dir, args.filename + "_val-split.csv"), index=False
    )
    test_data.to_csv(
        os.path.join(write_dir, args.filename + "_test-split.csv"), index=False
    )
    ...
    print(
        f"Total images for the checklist is {len(train_data) + len(val_data) + len(test_data)}."
    )
