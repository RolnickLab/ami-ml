...
def check_crop_size(bbox: list[str], image_pred_file: str, data_dir: str, min_crop_dim: int):
...
def count_crops_at_taxon_level(label: str, rank: str, exclusion_sp: list[str]):
    ...
    if rank == "ORDER": gt_moths += 1
    elif rank == "SPECIES" and label in exclusion_sp: skip_crops_excluded += 1
    elif rank == "SPECIES" and label not in exclusion_sp: gt_family+=1; gt_genus+=1; gt_species+=1; gt_moths += 1
    elif rank == "GENUS": gt_family+=1; gt_genus+=1; gt_moths += 1    
    else: gt_family+=1; gt_moths += 1
        

def histogram_at_taxon_level(label: str, rank: str, region:str, taxonomy_db: str, exclusion_sp: list[str]):
    """Calculations for moth crop counts for various classes at diff. taxon levels"""
    
    global hist_dict
    hierarchy = ["SPECIES", "GENUS", "FAMILY"]
    global neamerica_family, neamerica_genus,\
        neamerica_species, \
        camerica_family, camerica_genus,\
        camerica_species, \
        weurope_family, weurope_genus,\
        weurope_species    
    ...
    if rank == "ORDER": 
        ...
        if label not in exclusion_sp:
            ...
            if region == "NorthEasternAmerica": 
                if species not in neamerica_species: neamerica_species.append(species)
                if genus not in neamerica_genus: neamerica_genus.append(genus)
                if family not in neamerica_family: neamerica_family.append(family) 
            if region == "WesternEurope": 
                if species not in weurope_species: weurope_species.append(species)
                if genus not in weurope_genus: weurope_genus.append(genus)
                if family not in weurope_family: weurope_family.append(family)
            if region == "CentralAmerica": 
                if species not in camerica_species: camerica_species.append(species)
                if genus not in camerica_genus: camerica_genus.append(genus)
                if family not in camerica_family: camerica_family.append(family)     
...
def _helper_finegrained_accuracy(ground_truth: str, rank: str, prediction: list[list[str, str]], 
                         region: str, species_list: pd.DataFrame, taxonomy_db: pd.DataFrame, exclusion_sp: list[str]):
    """Calculate fine-grained classification accuracy for diff taxon levels"""

    global sp_top1, sp_top5, sp_top10, \
        gs_top1, gs_top5, gs_top10, \
        fm_top1, fm_top5, fm_top10

    # Region-wise global variables for accuracy
    global gt_family_neamerica, gt_genus_neamerica, gt_species_neamerica, \
        neamerica_sp_top1, neamerica_sp_top5, \
        neamerica_gs_top1, neamerica_gs_top5, \
        neamerica_fm_top1, neamerica_fm_top5, \
        gt_family_weurope, gt_genus_weurope, gt_species_weurope, \
        weurope_sp_top1, weurope_sp_top5, \
        weurope_gs_top1, weurope_gs_top5, \
        weurope_fm_top1, weurope_fm_top5, \
        gt_family_camerica, gt_genus_camerica, gt_species_camerica, \
        camerica_sp_top1, camerica_sp_top5, \
        camerica_gs_top1, camerica_gs_top5, \
        camerica_fm_top1, camerica_fm_top5
    ...
    if rank == "ORDER": 
        ...
        if ground_truth not in exclusion_sp:
            ...
            for pred in prediction:
                genus = species_list.loc[species_list["gbif_species"] == pred[0]]["genus"].values[0]
                family = species_list.loc[species_list["gbif_species"] == pred[0]]["family"].values[0]
                sp_pred_list.append(pred[0]); gs_pred_list.append(genus+" sp."); fm_pred_list.append(family)
            synonym_list = [species_list.loc[species_list["gbif_species"] == species]["search_species"].values[0] \
                    for species in sp_pred_list] # Get the synonym names
            
            # Add region-wise moth numbers
            if region == "NorthEasternAmerica": gt_species_neamerica+=1; gt_genus_neamerica+=1; gt_family_neamerica+=1
            if region == "WesternEurope": gt_species_weurope+=1; gt_genus_weurope+=1; gt_family_weurope+=1
            if region == "CentralAmerica": gt_species_camerica+=1; gt_genus_camerica+=1; gt_family_camerica+=1

            # Specis calculation
            if ground_truth in sp_pred_list[:1] or ground_truth in synonym_list[:1]: 
                sp_top1 += 1
                if region == "NorthEasternAmerica": neamerica_sp_top1 += 1
                if region == "WesternEurope": weurope_sp_top1 += 1
                if region == "CentralAmerica": camerica_sp_top1 += 1
            if ground_truth in sp_pred_list[:5] or ground_truth in synonym_list[:5]: 
                sp_top5 += 1
                if region == "NorthEasternAmerica": neamerica_sp_top5 += 1
                if region == "WesternEurope": weurope_sp_top5 += 1
                if region == "CentralAmerica": camerica_sp_top5 += 1
            if ground_truth in sp_pred_list[:10] or ground_truth in synonym_list[:10]: sp_top10 += 1

            # Genus calculation
            if gt_genus in gs_pred_list[:1]: 
                gs_top1 += 1
                if region == "NorthEasternAmerica": neamerica_gs_top1 += 1
                if region == "WesternEurope": weurope_gs_top1 += 1
                if region == "CentralAmerica": camerica_gs_top1 += 1
            if gt_genus in gs_pred_list[:5]: 
                gs_top5 += 1
                if region == "NorthEasternAmerica": neamerica_gs_top5 += 1
                if region == "WesternEurope": weurope_gs_top5 += 1
                if region == "CentralAmerica": camerica_gs_top5 += 1
            if gt_genus in gs_pred_list[:10]: gs_top10 += 1

            # Family calculation
            if gt_family in fm_pred_list[:1]: 
                fm_top1 += 1
                if region == "NorthEasternAmerica": neamerica_fm_top1 += 1
                if region == "WesternEurope": weurope_fm_top1 += 1
                if region == "CentralAmerica": camerica_fm_top1 += 1
            if gt_family in fm_pred_list[:5]: 
                fm_top5 += 1
                if region == "NorthEasternAmerica": neamerica_fm_top5 += 1
                if region == "WesternEurope": weurope_fm_top5 += 1
                if region == "CentralAmerica": camerica_fm_top5 += 1
            if gt_family in fm_pred_list[:10]: fm_top10 += 1  
        ...
        for pred in prediction:
            genus = species_list.loc[species_list["gbif_species"] == pred[0]]["genus"].values[0]
            family = species_list.loc[species_list["gbif_species"] == pred[0]]["family"].values[0]
            gs_pred_list.append(genus+" sp."); fm_pred_list.append(family)

        # Add region-wise moth numbers
        if region == "NorthEasternAmerica": gt_genus_neamerica+=1; gt_family_neamerica+=1
        if region == "WesternEurope": gt_genus_weurope+=1; gt_family_weurope+=1
        if region == "CentralAmerica": gt_genus_camerica+=1; gt_family_camerica+=1
        ...
        if ground_truth in gs_pred_list[:5]: 
            ...
            if region == "WesternEurope": weurope_gs_top5 += 1
            if region == "CentralAmerica": camerica_gs_top5 += 1
        if ground_truth in gs_pred_list[:10]: gs_top10 += 1
        ...
        # Add region-wise moth numbers
        if region == "NorthEasternAmerica": gt_family_neamerica += 1
        if region == "WesternEurope": gt_family_weurope+=1
        if region == "CentralAmerica": gt_family_camerica+=1
        ...
        if ground_truth in fm_pred_list[:5]: 
            ...
            if region == "WesternEurope": weurope_fm_top5 += 1
            if region == "CentralAmerica": camerica_fm_top5 += 1
        if ground_truth in fm_pred_list[:10]: fm_top10 += 1 
...
def _helper_combine_pred(prediction: list[list[str, str]]):
    ...
    return [[sp, conf] for sp, conf in sorted(list(pred_dict.items()), key=lambda item: item[1], reverse=True)]
    

def _helper_acc_alongside_conf(ground_truth: str, prediction: list[list[str, str]], rank: str):
    ...
    for thresh in range(10, top1_acc, 10):  
        ...
        if rank=="SPECIES": accuracy_w_conf_sp.loc["conf-total"]["conf-"+str(thresh)] += 1
        if rank=="GENUS": accuracy_w_conf_gs.loc["conf-total"]["conf-"+str(thresh)] += 1
        if rank=="FAMILY": accuracy_w_conf_fm.loc["conf-total"]["conf-"+str(thresh)] += 1

        # Add only correct counts
        if top1_class == ground_truth: 
            if rank=="SPECIES": accuracy_w_conf_sp.loc["correct"]["conf-"+str(thresh)] += 1 
            if rank=="GENUS": accuracy_w_conf_gs.loc["correct"]["conf-"+str(thresh)] += 1
            if rank=="FAMILY": accuracy_w_conf_fm.loc["correct"]["conf-"+str(thresh)] += 1 

    # Add counts for upwards rejected thresholds
    for upwards_thresh in range(thresh+10, 100, 10):
        if rank=="SPECIES": accuracy_w_conf_sp.loc["reject"]["conf-"+str(upwards_thresh)] += 1
        if rank=="GENUS": accuracy_w_conf_gs.loc["reject"]["conf-"+str(upwards_thresh)] += 1
        if rank=="FAMILY": accuracy_w_conf_fm.loc["reject"]["conf-"+str(upwards_thresh)] += 1


def accuracy_alongside_confidence(ground_truth: str, rank: str, prediction: list[list[str, str]], taxonomy_db: str):
...
def species_accuracy(gt_label: str, rank: str, prediction: list[list[str, str]], species_list: pd.DataFrame):
    ...
    if rank == "SPECIES":
        ...
        synonym_pred_top1 = species_list.loc[species_list["gbif_species"] == species_pred_top1]["search_species"].values[0]
        ...
        if gt_label == species_pred_top1 or gt_label == synonym_pred_top1: 
            if gt_label not in accuracy_sp.keys():
                ...
                accuracy_sp[gt_label][2] = round(accuracy_sp[gt_label][0]/accuracy_sp[gt_label][1], 2)
...
def fine_grained_classification_eval(data_dir: str, plot_dir: str, checklist: str, taxonomy: str, exclusion_sp_file: str,
                                     skip_small_crops: bool = False, min_crop_dim: int = 200):
    ...
    global gt_moths, gt_family, gt_genus, gt_species, \
        sp_top1, sp_top5, sp_top10, \
        gs_top1, gs_top5, gs_top10, \
        fm_top1, fm_top5, fm_top10
    ...
    global neamerica_family, neamerica_genus,\
        ...
        weurope_family, weurope_genus,\
        weurope_species    
    neamerica_family, neamerica_genus,\
        neamerica_species = [], [], []
    camerica_family, camerica_genus,\
        camerica_species = [], [], []
    weurope_family, weurope_genus,\
    ...
    global hist_dict 
    hist_dict, hist_dict["FAMILY"], hist_dict["GENUS"],\
        hist_dict["SPECIES"] = {}, {}, {}, {}
    ...
    if skip_small_crops:
        print(f"Crops with length less than {min_crop_dim}px are removed in this analysis.")
    ...
    pred_dir = os.path.join(data_dir, "ami_traps_dataset", "model_predictions", "all-architectures", model) # TEMP CHANGE
    ...
    for image_pred in image_pred_list:
        ...
        for bbox in pred_data:
            ...
            if gt_label!="Non-Moth" and gt_label!="Unidentifiable" \
            and gt_label!="Unclassified":
                ...
                _helper_finegrained_accuracy(gt_label, gt_rank, prediction, region, species_list, taxonomy_db, exclusion_sp)
...
def binary_classification_eval(data_dir: str, plot_dir: str, skip_small_crops: bool = True, min_crop_dim: int = 100):
    ...
    for image_pred in image_pred_list:
        ...
        for bbox in pred_data:
            ...
            if skip_small_crops:
                flag = check_crop_size(bbox_coord, image_pred, data_dir, min_crop_dim)
                if flag:  # skip this crop
                    if gt == "Non-Moth": skip_gt_nonmoths += 1
                    elif gt == "Unidentifiable": pass
                    else: skip_gt_moths += 1
                    continue
                else:
                    pass
            ...
            if gt == "Non-Moth":
                if pred == "nonmoth":
                    tn += 1; tn_cf.append(conf)
                if pred == "moth":
                    fp += 1; fp_cf.append(conf) 
                ...
                if pred == "moth":
                    tp += 1; tp_cf.append(conf)
                if pred == "nonmoth":
                    fn += 1; fn_cf.append(conf)
    ...
    sns.histplot(data=tp_cf,binwidth=10,binrange=(0, 100),kde=True, color="violet",ax=axs[0, 0],label="True Positives (TP)")
    sns.histplot(data=fp_cf,binwidth=10,binrange=(0, 100),kde=True,color="olive",ax=axs[0, 1],label="False Positives (FP)")
    sns.histplot(data=tn_cf,binwidth=10,binrange=(0, 100),kde=True,color="orange",ax=axs[1, 0],label="True Negatives (TN)")
    sns.histplot(data=fn_cf,binwidth=10,binrange=(0, 100),kde=True,color="teal",ax=axs[1, 1],label="False Negatives (FN)")
    axs[0, 0].legend(loc="upper left"); axs[0, 0].set(xlabel="Prediction confidence")
    axs[0, 1].legend(loc="upper left"); axs[0, 1].set(xlabel="Prediction confidence")
    axs[1, 0].legend(loc="upper left"); axs[1, 0].set(xlabel="Prediction confidence")
    axs[1, 1].legend(loc="upper left"); axs[1, 1].set(xlabel="Prediction confidence")
    ...
    for image_pred in image_pred_list:
        ...
        for bbox in pred_data:
            ...
            if gt == "Non-Moth":
                ...
                # Region statistics
                if region == "NorthEasternAmerica": neamerica_imgs += 1                                    
                elif region == "WesternEurope": weurope_imgs += 1
                elif region == "CentralAmerica": camerica_imgs += 1
                else:
...
if __name__ == "__main__":
    ...
    plot_dir = "/home/mila/a/aditya.jain/mothAI/cvpr2024/model_evaluation/plots"
    species_checklist = "/home/mila/a/aditya.jain/mothAI/species_lists/quebec-vermont-uk-denmark-panama_checklist_20231124.csv"
    taxonomy = "/home/mila/a/aditya.jain/scratch/cvpr2024_data/ami-taxonomy-joined-20240119.csv"
    ...
    fine_grained_classification_eval(data_dir, plot_dir, species_checklist, taxonomy, exclusion_sp_file)
