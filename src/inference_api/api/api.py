...
pipeline = PipelineConfig(
    ...
    stages=[
        PipelineStage(
            ...
            params=[
                PipelineStageParam(
                    name="Panama Moths", key="panama", category="Classifier"
                )
            ],
        )
    ],
...
async def readyz():
    if pipelines:
        ...
        return fastapi.responses.JSONResponse(
            status_code=503, content={"status": "pipelines unavailable"}
        )


@app.post(
    "/pipeline/process", tags=["services"]
)  # @TODO: Future change use @app.post("/{pipeline_name}/process/")
