...
def build_json(df, categ_map, drop_label):
    ...
    print(
        f"    [{drop_label}] kept {len(images):,} rows, dropped {n_unmapped} unmapped-species rows"
    )
...
def main():
    ...
    t(
        f"BQ fetch done. Rows: {len(df):,}. Bytes scanned: {bytes_billed/1e6:.1f} MB (~${bytes_billed/1e12*5:.4f})",
        fetch_start,
    )
    ...
    print(
        f"  Bad lats (placeholder values): {len(bad_lats)} → e.g. {sorted(bad_lats)[:5]}"
    )
    print(
        f"  Bad lons (placeholder values): {len(bad_lons)} → e.g. {sorted(bad_lons)[:5]}"
    )
    ...
    train = train[
        ~train["latitude"].isin(bad_lats) & ~train["longitude"].isin(bad_lons)
    ]
    ...
    print(
        f"  Species kept in train: {len(kept_species):,} (of {len(categ_map):,} total)"
    )
    ...
    for name, obj in [("train", train_json), ("val", val_json), ("test", test_json)]:
        ...
        with open(p, "w") as f:
            json.dump(obj, f)
        print(
            f'  Wrote {p}  ({p.stat().st_size/1e6:.1f} MB,  {len(obj["images"]):,} images)'
        )
